...
export function AppLaunchNode({ id, data, selected }: NodeProps<Node<AppLaunchData>>) {
  const launchTypeConfig = LAUNCH_TYPE_CONFIG[data.launchType]
  const LaunchIcon = launchTypeConfig.icon

  return (
    <BaseNode id={id} label={data.label} status={data.status} nodeKind="appLaunch" icon={Play} selected={selected}>
      ...
      <div className="flex items-center gap-1.5 min-w-0">
        ...
        <span className={cn("inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-medium leading-none", launchTypeConfig.className)}>
